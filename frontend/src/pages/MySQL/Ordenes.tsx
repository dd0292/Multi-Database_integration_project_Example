...
const MySQLOrdenes = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["mysql-ordenes", page],
    queryFn: async () => {
      const response = await api.get<{ data: MySQLOrden[]; total: number }>(
        `/mysql/ordenes?page=${page}&limit=${limit}`
      );
  ...
  const createMutation = useMutation({
    mutationFn: async (data: any) => {
      return api.post("/mysql/ordenes", data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["mysql-ordenes"] });
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-mysql">MySQL Órdenes</h1>
          <p className="text-muted-foreground mt-1">Manage sales orders in MySQL</p>
        </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <h3 className="font-semibold text-lg">Order #{orden.orden_id}</h3>
                        <Badge className={getChannelColor(orden.canal)}>{orden.canal}</Badge>
                      </div>
                      <p className="text-sm text-muted-foreground">
                        {orden.fecha} • {orden.items.length} items
                      </p>
                    </div>
                    <div className="text-right">
                      <div className="text-2xl font-bold text-mysql">{orden.total}</div>
